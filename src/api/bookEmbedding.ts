...
export const createBookEmbeddings = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  ...
  try {
    ...
    const books = await Book.find({}) as BookDocument[];
    ...
    const existingBookIdsArray = await mongoose.connection.collection("booksVectors").distinct("metadata._id");

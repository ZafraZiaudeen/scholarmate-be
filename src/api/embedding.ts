...
// Define an interface for the MCQ document
interface MCQDocument extends mongoose.Document {
  ...
  async embedDocuments(texts: string[]): Promise<number[][]> {
    return new Promise((resolve, reject) => {
      ...
      const pythonProcess = spawn(
        ...
        ["scripts/generate_embeddings.py", JSON.stringify(texts)],
        { cwd: process.cwd() }
      );
...
export const createEmbeddings = async (
  ...
  try {
    ...
    const mcqs = await MCQ.find({}) as MCQDocument[];
    ...
    const existingIds = new Set(
      (await mongoose.connection.collection("mcqVectors").distinct("_id")).map(id => id.toString())
    );
...
export const generateTextWithGroq = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const data = await response.json() as { choices: { text: string }[] }; // Type assertion

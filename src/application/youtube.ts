...
export const searchVideos = async (req: Request, res: Response, next: NextFunction) => {
	try {
		const {
			query = "HTML CSS JavaScript tutorial",
			maxResults = 12,
			pageToken,
			channelId,
			order = "relevance",
			useCurated = "true" // New parameter to control curated vs YouTube search
		} = req.query;
		...
		if (useCurated === "true") {
			try {
				...
				const curatedVideos = await CuratedVideo.find({
					isActive: true,
					$or: [
						{ title: { $regex: query, $options: 'i' } },
						{ description: { $regex: query, $options: 'i' } },
						{ tags: { $in: [(query as string).split(' ')] } }
					]
				})
				.populate('categoryId')
		...
		let detailsResponse;
		try {
			detailsResponse = await axios.get(`${YOUTUBE_BASE_URL}/videos`, {
				params: {
...
// Get playlist videos
export const getPlaylistVideos = async (req: Request, res: Response, next: NextFunction) => {
	try {
		...
		const response = await axios.get(`${YOUTUBE_BASE_URL}/playlistItems`, { params });
...
export const getCuratedRecommendations = async (topic: string): Promise<YouTubeVideo[]> => {
